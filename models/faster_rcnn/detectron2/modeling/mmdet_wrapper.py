...
from models.faster_rcnn.detectron2.layers import ShapeSpec
from models.faster_rcnn.detectron2.structures import BitMasks, Boxes, ImageList, Instances
from models.faster_rcnn.detectron2.utils.events import get_event_storage
...
class MMDetBackbone(Backbone):
    """
    Wrapper of mmdetection backbones to use in faster_rcnn.

    mmdet backbones produce list/tuple of tensors, while faster_rcnn backbones
    produce a dict of tensors. This class wraps the given backbone to produce
    output in faster_rcnn's convention, so it can be used in place of faster_rcnn
    backbones.
    """
...
class MMDetDetector(nn.Module):
    ...
    Input/output formats of this class follow faster_rcnn's convention, so a
    mmdetection model can be trained and evaluated in faster_rcnn.
    """
