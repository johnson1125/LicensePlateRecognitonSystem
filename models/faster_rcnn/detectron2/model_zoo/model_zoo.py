...
from models.faster_rcnn.detectron2.config import CfgNode, LazyConfig, get_cfg, instantiate
...
def get_checkpoint_url(config_path):
    ...
    Args:
        config_path (str): config file name relative to faster_rcnn's "configs/"
...
def get_config_file(config_path):
    ...
    cfg_file = pkg_resources.resource_filename(
        "models.faster_rcnn.detectron2.model_zoo", os.path.join("configs", config_path)
    )
...
def get(config_path, trained: bool = False, device: Optional[str] = None):
    ...
    Returns:
        nn.Module: a faster_rcnn model. Will be in training mode.

    Example:
    ::
        from faster_rcnn import model_zoo
