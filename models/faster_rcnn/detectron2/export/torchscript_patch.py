...
def patch_instances(fields):
    """
    A contextmanager, under which the Instances class in faster_rcnn is replaced
    by a statically-typed scriptable class, defined by `fields`.
    See more in `scripting_with_instances`.
    """

    with tempfile.TemporaryDirectory(prefix="faster_rcnn") as dir, tempfile.NamedTemporaryFile(
...
import faster_rcnn
from faster_rcnn.structures import Boxes, Instances
...
def patch_builtin_len(modules=()):
    """
    Patch the builtin len() function of a few faster_rcnn modules
    to use __len__ instead, because __len__ does not convert values to
    integers and therefore is friendly to tracing.

    Args:
        modules (list[stsr]): names of extra modules to patch len(), in
            addition to those in faster_rcnn.
    """
    ...
    with ExitStack() as stack:
        MODULES = [
            "faster_rcnn.modeling.roi_heads.fast_rcnn",
            "faster_rcnn.modeling.roi_heads.mask_head",
            "faster_rcnn.modeling.roi_heads.keypoint_head",
        ] + list(modules)
...
def patch_nonscriptable_classes():
    """
    Apply patches on a few nonscriptable faster_rcnn classes.
