...
class Caffe2Tracer:
    """
    Make a faster_rcnn model traceable with Caffe2 operators.
    This class creates a traceable version of a faster_rcnn model which:
    ...
    def __init__(self, cfg: CfgNode, model: nn.Module, inputs):
        """
        Args:
            cfg (CfgNode): a faster_rcnn config used to construct caffe2-compatible model.
            model (nn.Module): An original pytorch model. Must be among a few official models
                in faster_rcnn that can be converted to become caffe2-compatible automatically.
...
class Caffe2Model(nn.Module):
    ...
    def __call__(self, inputs):
        """
        An interface that wraps around a Caffe2 model and mimics faster_rcnn's models'
        ...
        on faster_rcnn in order to convert to faster_rcnn's output format.
        """
