...
def assemble_rcnn_outputs_by_name(image_sizes, tensor_outputs, force_mask_on=False):
    ...
    to faster_rcnn's format (i.e. list of Instances instance).
    This only works when the model follows the Caffe2 faster_rcnn's naming convention.
...
class Caffe2MetaArch(Caffe2Compatible, torch.nn.Module):
    ...
    def __init__(self, cfg, torch_model, enable_tensor_mode=True):
        ...
        Args:
            cfg (CfgNode):
            torch_model (nn.Module): the faster_rcnn model (meta_arch) to be
                converted.
        """
        super().__init__()
    ...
    def get_caffe2_inputs(self, batched_inputs):
        ...
        Args:
            batched_inputs (list[dict]): inputs to a faster_rcnn model
