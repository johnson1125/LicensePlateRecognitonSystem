...
class FlopCountAnalysis(fvcore.nn.FlopCountAnalysis):
    """
    Same as :class:`fvcore.nn.FlopCountAnalysis`, but supports faster_rcnn models.
    """
...
def flop_count_operators(model: nn.Module, inputs: list) -> typing.DefaultDict[str, float]:
    ...
    detection models in faster_rcnn.
    ...
    Args:
        model: a faster_rcnn model that takes `list[dict]` as input.
        inputs (list[dict]): inputs to model, in faster_rcnn's standard format.
