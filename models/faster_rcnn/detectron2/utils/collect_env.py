...
def collect_env_info():
    ...
    try:
        ...
        data.append(("faster_rcnn", "failed to import"))
    except AttributeError:
        data.append(("faster_rcnn", "imported a wrong installation"))

    try:
        import detectron2._C as _C
    except ImportError as e:
        data.append(("faster_rcnn._C", f"not built correctly: {e}"))
        ...
        if has_cuda and sys.platform != "win32":
            try:
                so_file = importlib.util.find_spec("faster_rcnn._C").origin
            except (ImportError, AttributeError):
                pass
            else:
                data.append(
                    (
                        "faster_rcnn arch flags",
