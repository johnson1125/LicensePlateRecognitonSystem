...
def patch_instances(fields):
    """
    A contextmanager, under which the Instances class in detectron is replaced
    by a statically-typed scriptable class, defined by `fields`.
    See more in `scripting_with_instances`.
    """

    with tempfile.TemporaryDirectory(prefix="detectron") as dir, tempfile.NamedTemporaryFile(
...
def patch_builtin_len(modules=()):
    """
    Patch the builtin len() function of a few detectron modules
    to use __len__ instead, because __len__ does not convert values to
    integers and therefore is friendly to tracing.

    Args:
        modules (list[stsr]): names of extra modules to patch len(), in
            addition to those in detectron.
    """
    ...
    with ExitStack() as stack:
        MODULES = [
            "detectron.modeling.roi_heads.fast_rcnn",
            "detectron.modeling.roi_heads.mask_head",
            "detectron.modeling.roi_heads.keypoint_head",
        ] + list(modules)
...
def patch_nonscriptable_classes():
    """
    Apply patches on a few nonscriptable detectron classes.
