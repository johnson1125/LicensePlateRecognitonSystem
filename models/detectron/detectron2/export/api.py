...
class Caffe2Tracer:
    """
    Make a detectron model traceable with Caffe2 operators.
    This class creates a traceable version of a detectron model which:
    ...
    def __init__(self, cfg: CfgNode, model: nn.Module, inputs):
        """
        Args:
            cfg (CfgNode): a detectron config used to construct caffe2-compatible model.
            model (nn.Module): An original pytorch model. Must be among a few official models
                in detectron that can be converted to become caffe2-compatible automatically.
...
class Caffe2Model(nn.Module):
    ...
    def __call__(self, inputs):
        """
        An interface that wraps around a Caffe2 model and mimics detectron's models'
        ...
        on detectron in order to convert to detectron's output format.
        """
