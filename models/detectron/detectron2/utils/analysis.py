...
class FlopCountAnalysis(fvcore.nn.FlopCountAnalysis):
    """
    Same as :class:`fvcore.nn.FlopCountAnalysis`, but supports detectron models.
    """
...
def flop_count_operators(model: nn.Module, inputs: list) -> typing.DefaultDict[str, float]:
    ...
    Args:
        model: a detectron model that takes `list[dict]` as input.
        inputs (list[dict]): inputs to model, in detectron's standard format.
