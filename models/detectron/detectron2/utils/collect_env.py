...
def collect_env_info():
    ...
    try:
        ...
        data.append(("detectron", "failed to import"))
    except AttributeError:
        data.append(("detectron", "imported a wrong installation"))

    try:
        import detectron2._C as _C
    except ImportError as e:
        data.append(("detectron._C", f"not built correctly: {e}"))
        ...
        if has_cuda and sys.platform != "win32":
            try:
                so_file = importlib.util.find_spec("detectron._C").origin
            except (ImportError, AttributeError):
                pass
            else:
                data.append(
                    (
                        "detectron arch flags",
